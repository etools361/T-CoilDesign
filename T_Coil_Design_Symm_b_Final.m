...
%--------------------------------------------------------------------------
ConstR_TEST = 0;
syms C_b L_a L_b L_m C R s w_n zeta b R_p R_b R_s R_c R_a
A  = R_c/(1+s*R_c*C_b);
B  = s*L_a;
CC = s*L_a;
D  = s*L_m+1/(s*C)+R_s;
...
FF1 = collect(nn,s);
coef = coeffs(FF1, s);
y = solve(coef(1)==0, coef(2)==0, coef(3)==0, L_m, L_a, R_c);
kk = 1;
L_a0 = simplify(y.L_a(kk))
R_c0 = simplify(y.R_c(kk))
...
y = solve(Hf2(end)==0, Hf3(end)==0,Hf4(end)==0,...
    L_a0==L_a,R_c0==R_c,L_m0==L_m, R_b, L_a, L_b, L_m, w_n, C_b, R_c);
kk = 1;
...
R_c1  = simplify(y.R_c(kk))% C_b = -(C*(R*(R_p^3/(R_p - R + R_s))^(1/2) - R_p*(R_p^3/(R_p - R + R_s))^(1/2) - R_s*(R_p^3/(R_p - R + R_s))^(1/2) + (-R_p*(R_p + R_s)*(R_p*R_s + 2*R*(R_p^3/(R_p - R + R_s))^(1/2) - 2*R_p*(R_p^3/(R_p - R + R_s))^(1/2) - 2*R_s*(R_p^3/(R_p - R + R_s))^(1/2) + R_p^2))^(1/2)))/R^2
