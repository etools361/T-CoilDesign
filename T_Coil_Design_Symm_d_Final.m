...
AA = R_c; BB = R_a; CCC = R_a; DD = R_p+R_s; EE = R; 
...
Z_D = R_p/(1+s*C*R_p);
% ZG  = R;
CellName = {     zeta,  R,     C, R_p, R_s};
CellVal  = [sqrt(2)/2, 50, 4e-12, 500, 10];
...
R_c0 = simplify(y.R_c(kk))% R_c = (4*R^2*R_p)/(R^2 + 4*R_p*R_s)
...
R_c1  = simplify(y.R_c(kk))% R_c = (4*R^2*R_p)/(R^2 + 4*R_p*R_s)
L_m1  = simplify(y.L_m(kk))% L_m = (C*R^2*zeta^2)/2 - (C*zeta*((R^2 + 2*R*R_p + 4*R_p^2 + 4*R_s*R_p)*(R^2*zeta^2 - R^2 + 2*R*R_p*zeta^2 - 2*R*R_p + 4*R_p^2*zeta^2 + 4*R_s*R_p*zeta^2 - 4*R_s*R_p))^(1/2))/2 - (C*R^2)/2 + 2*C*R_p^2*zeta^2 - (C*R*R_p)/2 - C*R_p*R_s + C*R*R_p*zeta^2 + 2*C*R_p*R_s*zeta^2
R_a1  = simplify(y.R_a(kk))% R_a = R^2/(2*R_p)
w_n1  = simplify(y.w_n(kk))% w_n = (R^2*zeta + 4*R_p^2*zeta + ((R^2 + 2*R*R_p + 4*R_p^2 + 4*R_s*R_p)*(R^2*zeta^2 - R^2 + 2*R*R_p*zeta^2 - 2*R*R_p + 4*R_p^2*zeta^2 + 4*R_s*R_p*zeta^2 - 4*R_s*R_p))^(1/2) + 2*R*R_p*zeta + 4*R_p*R_s*zeta)/(C*R_p*(R^2 + 2*R_p*R + 4*R_p*R_s))
C_b1  = simplify(y.C_b(kk))% C_b = -(C*(2*R*R_p + 4*R_p*R_s + R^2 + 2*zeta*((R^2 + 2*R*R_p + 4*R_p^2 + 4*R_s*R_p)*(R^2*zeta^2 - R^2 + 2*R*R_p*zeta^2 - 2*R*R_p + 4*R_p^2*zeta^2 + 4*R_s*R_p*zeta^2 - 4*R_s*R_p))^(1/2) - 2*R^2*zeta^2 - 8*R_p^2*zeta^2 - 4*R*R_p*zeta^2 - 8*R_p*R_s*zeta^2))/(4*R^2)
...
Gain0=simplify(subs(ZG, {R_a, R_c}, {R_a1,R_c1}));
Gain = subs(Gain0/R, CellName, CellVal);
fprintf('Gain=%0.3f\n', vpa(Gain));

% 验证
fprintf('---Symm.验证公式---\n');
Rp = CellVal(4); Rs = CellVal(5); R = CellVal(2); C = CellVal(3); zeta = CellVal(1);
La =  (C * R^2) / 2;
Lb = La;
Ra =  (R^2) / (2 * Rp);
Rb = Ra;
Rc = (4 * R^2 * Rp) / (R^2 + 4 * Rp * Rs);
T1 = (R^2 / Rp^2) + 2 * (R / Rp) + 4 * (Rs / Rp);
Cb = (C / 4) * (Rp^2 / R^2) * (2 * zeta^2 * (T1 + 4) - 2 * zeta * sqrt((T1 + 4) * ((T1 + 4) * zeta^2 - T1)) - T1);
Lm = R^2 * Cb - La / 2;
wn = ((T1 + 4) * zeta + sqrt((T1 + 4) * ((T1 + 4) * zeta^2 - T1))) / (C * Rp * T1);
Gain = 4 / (((R / Rp) + 1)^2 + 4 * (Rs / Rp) + 3);
...
fprintf('f3dB=%0.3fGHz(%0.3f/RC)\n', (w_n_x)/2/pi*1e-9, w_n_x*CellVal(2)*CellVal(3));
fprintf('Gain=%0.3f\n', (Gain));
